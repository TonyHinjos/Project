    ...
    $.each($('#' + tab_id).children('div:first').children('div'), function (index, obj) {
        for (var j = 0; j < key_arr.length; j++) {
            ...
            if ($(obj).has(err_id).length) {
                ...
                if ($('[name="' + key_arr[j].key + '"]')[0]) {
                    $('[name="' + key_arr[j].key + '"]')[0].focus();
                }
        ...
        $("#" + field_id).autocomplete({
            source: function (request, response) {
                ...
                $.each(input_arr, function (key, value) {
                    dataObj[key] = (value) ? value : $.trim(request.term);
                });
                $.ajax({
                    url: dest_url,
                    type: 'POST',
                ...
                if (element_obj.admin == '1') {
                    switch (element_type) {
                        case 'file':
                            ...
                            total_obj[tb] = {
                                'dynamicUpload': {
                                    upload_field: element_obj.name + '_upload',
                                    upload_value_field: element_obj.name,
                                    setting_info: setting_info,
                                    language_array: languageArray(),
                                    save_url: settingObj.form_action.dynamic_field_generator.uploader_upload_url,
                                    remove_url: settingObj.form_action.dynamic_field_generator.uploader_remove_url
                                }, 'calendar': ''
                            };
                            tb++;
                            break;
...
function commonResetFormFields(settingObj) {
    if ($("#" + settingObj.form_action.form_id)[0]) {
...
function commonGrid(settingObj) {
    ...
    var grid = $("#" + settingObj.grid_id).kendoGrid({
        ...
        pageable: (settingObj.pageable) ? settingObj.pageable : {
            refresh: true,
            pageSizes: true,
            buttonCount: 3,
...
function detailInit(e, settingObj) {
    ...
    var subGrid = $("<div/>").appendTo(e.detailCell).kendoGrid({
        ...
        dataSource: {
            ...
            filter: {
                field: settingObj.detailInit_action.detailInit_filter_field_name,
                operator: "eq",
                value: e.data[settingObj.detailInit_action.detailInit_filter_field_id]
            }
        },
...
function commonMsgDialog(settingObj, msg) {
    $('#' + settingObj.messages.common_msg_dialog_id).html(msg).dialog({
        ...
        resizable: true,
        buttons: [{
            text: settingObj.messages.common_ok, click: function () {
    ...
    if (settingObj.search_action) {
        $('a.' + settingObj.search_action.search_form_btn).click(function () {
            if (((settingObj.search_action.search_validator === true) && (settingObj.search_action.search_validator_function() === true)) || (settingObj.search_action.search_validator == false) || (settingObj.search_action.search_validator == null)) {
                ...
                if (settingObj.common_action && settingObj.common_action.toolbar_button_loader && settingObj.search_action.show_loader_on_search_button) {
                    $(self).html(settingObj.common_action.toolbar_button_loader);
                }
